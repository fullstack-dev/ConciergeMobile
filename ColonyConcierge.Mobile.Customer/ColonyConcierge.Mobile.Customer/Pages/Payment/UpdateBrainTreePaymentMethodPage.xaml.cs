...
namespace ColonyConcierge.Mobile.Customer
{
    ...
    public partial class UpdateBrainTreePaymentMethodPage : ContentPageBase
	{
		//AddressFacade mAddressFacade = new AddressFacade();
		private Func<UpdateBrainTreePaymentMethodPage, bool> Saved;
		private IAppServices mAppServices;
		private string FormatDate = "MM/yyyy";
		public CreditCardData CreditCardData { get; set; }

		public UpdateBrainTreePaymentMethodPage(CreditCardData creditCardData, Func<UpdateBrainTreePaymentMethodPage, bool> saved = null)
		{
			Saved = saved;
			...
			cardNickNameEntry.Text = creditCardData.AccountNickname;
			CheckBoxIsPreferred.Checked = creditCardData.IsPreferred;

			//creditCardNumberEntry.Text = string.Empty;
			//EntryMMYY.Text = string.Empty;
			//EntryPostalCode.Text = string.Empty;
			//EntryCvv.Text = string.Empty;
			...
			{
				if (e.NewTextValue != null && e.NewTextValue != null && (e.OldTextValue == null || e.NewTextValue.Length > e.OldTextValue.Length))
				{
		...
		public bool CheckValidator()
		{
			...
			if (!cvvValidator.IsValid || !postalCodeValidator.IsValid || 
				...
				mAppServices.GetNoncenBrainTree(brainTreeCard, async (obj) =>
				{
					if (obj is string && !string.IsNullOrEmpty(obj as string))
					{
						try
						{
							...
							var result = await Shared.APIs.IAccounts.BtUpdatePaymentMethod_Async(userModel.AccountID, paymentAccountData.ID, paymentAccountData);
							if (result)
							{
								CreditCardData = paymentAccountData;
								if (Saved != null)
								{
									handle = Saved.Invoke(this);
								}
